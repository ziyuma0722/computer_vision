...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    x = np.linspace(border, w - border - 1, num = nPointsX, dtype=int)
    y = np.linspace(border, h - border - 1, num = nPointsY, dtype=int)

    vPoints = np.transpose([np.repeat(x, nPointsY), np.tile(y, nPointsX)])
    ...
    for i in range(len(vPoints)):
        ...
        for cell_y in range(-2, 2):
            for cell_x in range(-2, 2):
                ...
                id_x = np.arange(start_x, end_x)
                id_y = np.arange(start_y, end_y)
                
                angle = np.arctan2(grad_y[id_y, id_x], grad_x[id_y, id_x]) % (2 * np.pi)

                #first separate into 16 bins, then assign the two bins to the bin of closest direction among the eight directions.
                #e.g. for the direction of the x-coordinate, it contains bin 0 and 15.
                idx_angle = (np.round(angle * 16 / (2 * np.pi))).astype(np.intc)
                idx_angle[np.where(idx_angle == 16)] = 0
                idx_angle[np.where((idx_angle == 0))] = 0
                idx_angle[np.where((idx_angle == 15))] = 0
                idx_angle[np.where((idx_angle == 1))] = 1
                idx_angle[np.where((idx_angle == 2))] = 1
                idx_angle[np.where((idx_angle == 3))] = 2
                idx_angle[np.where((idx_angle == 4))] = 2
                idx_angle[np.where((idx_angle == 5))] = 3
                idx_angle[np.where((idx_angle == 6))] = 3
                idx_angle[np.where((idx_angle == 7))] = 4
                idx_angle[np.where((idx_angle == 8))] = 4
                idx_angle[np.where((idx_angle == 9))] = 5
                idx_angle[np.where((idx_angle == 10))] = 5
                idx_angle[np.where((idx_angle == 11))] = 6
                idx_angle[np.where((idx_angle == 12))] = 6
                idx_angle[np.where((idx_angle == 13))] = 7
                idx_angle[np.where((idx_angle == 14))] = 7

                hist, bin_edges = np.histogram(idx_angle, bins=[0,1,2,3,4,5,6,7,8])
