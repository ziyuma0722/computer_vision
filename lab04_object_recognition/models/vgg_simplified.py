...
class Vgg(nn.Module):
    def __init__(self, fc_layer=512, classes=10):
        ...
        self.conv_block1 = nn.Sequential(nn.Conv2d(3, 64, 3, padding=1), nn.ReLU(), nn.MaxPool2d(2, stride=2))
        self.conv_block2 = nn.Sequential(nn.Conv2d(64, 128, 3, padding=1), nn.ReLU(), nn.MaxPool2d(2, stride=2))
        self.conv_block3 = nn.Sequential(nn.Conv2d(128, 256, 3, padding=1), nn.ReLU(), nn.MaxPool2d(2, stride=2))
        self.conv_block4 = nn.Sequential(nn.Conv2d(256, 512, 3, padding=1), nn.ReLU(), nn.MaxPool2d(2, stride=2))
        self.conv_block5 = nn.Sequential(nn.Conv2d(512, 512, 3, padding=1), nn.ReLU(), nn.MaxPool2d(2, stride=2))
        self.classifier = nn.Sequential(nn.Linear(self.fc_layer, 256), nn.ReLU(), nn.Dropout(), nn.Linear(in_features=256, out_features=10))      
    ...
    def forward(self, x):
        ...
        #use torch.squeeze to turn [bs, 512, 1, 1] to [bs, 512]
