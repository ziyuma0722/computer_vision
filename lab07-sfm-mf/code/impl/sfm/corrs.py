...
def UpdateReconstructionState(new_points3D, corrs, points3D, images):
  ...
  old_size = points3D.shape[0]
  points3D = np.append(points3D, new_points3D, 0)

  for im_name in corrs:
    indices = list(range(corrs[im_name][1], corrs[im_name][2]))
    indices_with_offset = [i + old_size for i in indices]
    images[im_name].Add3DCorrs(corrs[im_name][0], indices_with_offset)
